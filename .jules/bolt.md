...
## 2025-03-05 - Debounce URL state and localStorage sync
**Learning:** High-frequency interactions (like dragging a slider) cause rapid state updates. If these updates directly trigger synchronous calls to `window.history.replaceState` and `localStorage.setItem`, it blocks the main thread, hits browser rate limits, and causes severe UI stuttering.
**Action:** Always wrap state synchronization logic to external APIs (URL/Storage) in a debounced `setTimeout` (e.g., 300ms) when the state is bound to high-frequency UI inputs.
